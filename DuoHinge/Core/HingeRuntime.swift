...
final class HingeRuntime {
    ...
    @ObservationIgnored var simulatedAngle = 90.0 {
        didSet {
            simulatedSampleTime = ProcessInfo.processInfo.systemUptime
            refresh()
        }
    }
    @ObservationIgnored private var simulatedSampleTime = ProcessInfo.processInfo.systemUptime
    ...
    var angle: Double { sensor.isAvailable ? sensor.angle : simulatedAngle }
    ...
    func refresh() {
        ...
        if target > 0.0001, capture.pixelBuffer != nil {
            lastProgress = target
            overlay.update(
                angle: angle,
                sampleTime: sensor.isAvailable ? sensor.sampleTime : simulatedSampleTime,
