...
struct ContentView: View {
    ...
    var body: some View {
        if !runtime.permissions.hasScreenRecordingPermission {
            Button("⚠️ Screen Recording Permission Required…") {
        ...
        if !runtime.sensor.isAvailable {
            Menu("Simulate Desktop Effect") {
                Picker(
                    "Lid angle",
                    selection: Binding(
                        get: { runtime.simulatedAngle },
                        set: { runtime.simulatedAngle = $0 })
                ) {
                    Text("45° — Closed").tag(45.0)
                    Text("75° — Partly Open").tag(75.0)
                    Text("90° — Upright").tag(90.0)
                    Text("120° — Open").tag(120.0)
                }.pickerStyle(.inline)
            }
        }
